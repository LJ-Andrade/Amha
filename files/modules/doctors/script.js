...
{
	var brokers = "0";
	$("#select_broker").next('span').children('span').children('span').children('ul').children('.select2-selection__choice').each(function(){
		var optionName = $(this).attr("title");
		$("#select_broker").children("option").each(function(){
...
$(function(){
	$("#BtnCreate,#BtnCreateNext").on("click",function(e){
		...
		if(validate.validateFields('new_company_form'))
		{
			...
			if($("#sex").val()=='M')
			{
				confirmText += " el doctor ";
			}else{
				confirmText += " la doctora ";
			}

			confirmText += "'"+$("#first_name").val()+" "+$("#last_name").val()+"'";

			alertify.confirm(utf8_decode('¿Desea '+confirmText+' ?'), function(e){
				if(e)
				{
					var process		= '../../library/processes/proc.common.php?object=Doctor';
					if(BtnID=="BtnCreate")
					{
...
$(function(){
	...
	$("#image").change(function(){
		var process		= '../../library/processes/proc.common.php?action=newimage&object=Doctor';
		var haveData	= function(returningData)
		{
...
function addBranchModal()
{
		...
		var process		= '../../library/processes/proc.common.php?object=Doctor&action=Getbranchmodal';
		var haveData	= function(returningData)
		{
			//console.log(returningData);
...
function addBranch()
{
	if(!$("#total_branches").val()) $("#total_branches").val(0);
	var id = parseInt($("#total_branches").val())+1;
	$("#total_branches").val(id);
	var name = 'Consultorio '+(id-1);
	var img = '../../../skin/images/body/pictures/office.png';
	var html = '<div id="branch_row_'+id+'" class="row branch_row listRow2" style="margin:0px!important;"><div class="col-lg-1 col-md-2 col-sm-3 flex-justify-center hideMobile990"><div class="listRowInner"><img class="img" style="margin-top:5px!important;" src="'+img+'" alt="Consultorio" title="Consultorio"></div></div><div class="col-lg-9 col-md-7 col-sm-5 flex-justify-center"><span class="listTextStrong" style="margin-top:15px!important;" id="branch_row_name_'+id+'">'+name+'</span></div><div class="col-lg-1 col-md-2 col-sm-3 flex-justify-center"><button type="button" branch="'+id+'" id="EditBranch'+id+'" class="btn btnBlue EditBranch"><i class="fa fa-pencil"></i></button>&nbsp;<button type="button" id="DeleteBranch'+id+'" branch="'+id+'" class="btn btnRed DeleteBranch"><i class="fa fa-trash"></i></button></div></div>';
...
function SaveBranchEdition()
{
	$(".SaveBranchEdition").click(function(e){
		...
		if(validate.validateFields('branch_form_'+id) && validateMap(id))
		{
			...
			if($("#floor_"+id).val())
				floor = ' '+$("#floor_"+id).val()+'°';
			if($("#apartment_"+id).val())
				apartment = ' '+$("#apartment_"+id).val();
			$("#branch_row_name_"+id).html(''+$("#address_"+id).val()+floor+apartment+', '+$("#map"+id+"_zone").val()+'. '+$("#map"+id+"_province_short").val());
			$("#BranchTitle"+id).html('Editar Consultorio '+$("#address_"+id).val());
		}
		ShowErrorMapDiv();
		return false;
	});
